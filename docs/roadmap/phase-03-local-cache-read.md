...
| 5 | Stock by category for the center | Built on `GET /v1/dashboard/national`, which counts **sealed** boxes and is scoped to the caller's center by the server. It says so on screen, and says «de todos los centros» to national administration, because the same route answers both. | 🟠 Medium | ✅ Done |
