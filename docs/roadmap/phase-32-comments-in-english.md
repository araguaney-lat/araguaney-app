...
| 2 | `lib/core` | The layer everything else reads: the failure taxonomy, the queue, the theme, the session. Its comments are the ones most often quoted by the others. | 🔴 High | ⬜ Pending |
| 3 | `lib/features`, by feature | Capture and its queue first — the four invariants are the densest reasoning here. | 🔴 High | ⬜ Pending |
...
| 6 | A check that keeps it | Something in CI that fails on a Spanish comment, so the drift cannot happen twice. | 🟠 Medium | ⬜ Pending |
