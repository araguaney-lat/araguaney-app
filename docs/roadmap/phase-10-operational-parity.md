...
| 15 | Reading a medicine label from a photo | `POST /v1/intakes/read-label` takes a photo and returns the batch and expiry printed on the box, so the two fields most often mistyped can be filled from the label instead. The panel calls it since 2026-09-04; the application does not, and cannot yet: `mobile_scanner` reads codes and does not capture images, so this needs a photo dependency, camera permission strings for both platforms, a regenerated client, and a store release. **The rule for what a reading may fill lives outside the form** (`frontend/src/lib/label-reading.ts`): only empty fields, and only an `expiry_date` the date input can display. The application implements that same rule rather than a second one. Deliberately behind the panel: a store release is expensive to correct, so the design is validated where a merge fixes it. Needs a measurement first — the capability is off until its evaluation set clears its threshold. | 🟠 Medium | ⬜ Pending |
