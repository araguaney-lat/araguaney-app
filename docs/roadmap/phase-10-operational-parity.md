...
| 12 | Reports for the center | Seven report routes scoped by `tenant_scope`: activity, shrinkage, summary, countries, by-center, by-category and a CSV export. None is used: the category screen moved to the dashboard aggregate, which counts sealed boxes instead of a thirty-day window. | 🟠 Medium | ⬜ Pending |
...
| 14 | Better search when capturing | `GET /v1/catalog/search`, `GET /v1/catalog/barcode/{gtin}` and `GET /v1/intakes/donors/search` exist and the capture form uses none of them: it lists product types and captures donors by hand.  **Half done on 2026-08-21**: `catalog/barcode` is used by [Phase 13](phase-13-product-barcode.md). `catalog/search` and the donor search are still unused. | 🟢 Low | 🟨 Partial |
